...
check(node(bridge, "Update Capture Progress").alwaysOutputData === true && node(bridge, "Respond Accepted Progress").parameters.responseBody.includes("Shape Accepted Progress Response") && node(bridge, "Shape Accepted Progress Response").parameters.jsCode.includes("cancelled:false"), "accepted progress must preserve and return the exact acknowledgement after the data-table update");
